...
export class DistributorController {
  ...
  getProductsDashboard(@CurrentUser('distributor') distributor: any) {
    return this.distributorService.getProductsDashboard(distributor.id);
  }

  @Get('orders')
  @ApiOperation({ summary: 'Distributor buyurtmalari' })
  getOrders(@CurrentUser('distributor') distributor: any, @Query('status') status?: string) {
    return this.distributorService.getOrders(distributor.id, status);
  }

  @Post('orders/:orderId/accept')
  @ApiOperation({ summary: 'Buyurtmani qabul qilish' })
  acceptOrder(@Param('orderId') orderId: string, @CurrentUser('distributor') distributor: any) {
